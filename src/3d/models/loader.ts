...
/** 应用阴影到模型子树 */
const applyShadows = (obj: THREE.Object3D, opts?: LoadOpts): void => {
  if (!opts) {
    return;
  }
  obj.traverse((child) => {
...
export const assetProvider: ModelProvider = {
  ...
  async load(src: string, opts?: LoadOpts): Promise<THREE.Object3D> {
    // asset:windmill → modelRegistry[windmill] 的带 hash URL
 ...
 */
export const loadModel = async (src: string, opts?: LoadOpts): Promise<THREE.Object3D> => {
