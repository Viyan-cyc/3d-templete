/**
 * ============================================================
 *  hunyuan.ts — 混元 3D 模型生成 provider（单次生成 + 缓存）
 *
...
import type { ModelProvider } from './loader';

/** 混元生成产物 */
interface HunyuanResult {
  // 混元返回 glb bytes
  bytes?: ArrayBuffer
  // 混元返回远程 url（不再缓存 bytes，只记 src）
  src?: string
}
...
export const normalizeKey = (src: string): string => decodeURIComponent(src.replace(/^hunyuan:/i, '')).trim().toLowerCase();
 ...
 */
const callHunyuanGenerate = async (key: string): Promise<HunyuanResult> => {
  // TODO: 接入真实混元 API
  //   const res = await fetch('https://hunyuan.../generate', { body: JSON.stringify({ prompt: key }) })
  //   return { bytes: await res.arrayBuffer() }
  console.warn(`[hunyuan-provider] 混元未接入（占位），prompt="${key}"，将回落 mesh 兜底`);
  throw new Error('HUNYUAN_NOT_IMPLEMENTED');
};

/** 把 glb bytes 解析成 THREE.Object3D 原型 */
const parseGlbBytes = (bytes: ArrayBuffer): Promise<THREE.Object3D> => new Promise<GLTF>((resolve, reject) => {
  getGltfLoader().parse(bytes, '', resolve, reject);
}).then((gltf) => gltf.scene);

/** 加载远程 src（混元返回 url 的情况） */
const loadRemoteSrc = (src: string): Promise<THREE.Object3D> =>
 ...
 */
const loadFromDisk = (): Promise<ArrayBuffer | null> =>
...
export const hunyuanProvider: ModelProvider = {
  ...
  async load(src: string): Promise<THREE.Object3D> {
    const key = normalizeKey(src);

    // ① 已完成：直接复用缓存
    const hit = cache.get(key);
    if (hit?.status === 'done') {
      return hit.prototype ? hit.prototype.clone(true) : parseGlbBytes(hit.bytes!);
    }
    ...
    const p = (async () => {
      // 先查磁盘
      const diskBytes = await loadFromDisk();
      if (diskBytes) {
      ...
      try {
        ...
        // 上层 resolver catch → 落 mesh 兜底 + SCENE_ERROR
        throw err;
      } finally {
        inFlight.delete(key);
      }
