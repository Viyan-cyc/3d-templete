...
import type { ComponentContext, ObjectIndex } from '../managers/component/ComponentManager'
...
export function buildObjects(
  ...
  if (objects.length === 0) return nodeMap
  ...
  for (const oc of objects) {
    ...
    if (node) {
      ...
      if (oc.type === 'component') registerComponentChildren(node, nodeMap)
    } else {
      // 解析失败(如未知 geometry/src/builder 名):打 warn 便于定位。
      skippedCount++
      console.warn(
        `[objects] 无法创建物体，跳过: id=${oc.id} type=${oc.type}` +
  ...
  if (debug) {
    console.log(
      `[objects] 场景构建完成: 创建 ${createdCount}/${objects.length} 物体` +
...
function registerComponentChildren(node: THREE.Object3D, index: ObjectIndex): void {
  node.traverse((child) => {
    if (child.userData?.id && child !== node) index.set(child.userData.id, child)
  })
}
 ...
 */
function markZones(objects: LiveDataObject[], index: ObjectIndex): void {
  const rootIds = new Set<string>()
  for (const o of objects) if (!o.parentId) rootIds.add(o.id)
  const zoneIds = new Set<string>()
  for (const o of objects) if (o.__zone) zoneIds.add(o.id)
  if (zoneIds.size === 0) {
    for (const o of objects) if (o.parentId && rootIds.has(o.parentId)) zoneIds.add(o.id)
  }
...
function markZoneSingle(def: LiveDataObject, node: THREE.Object3D, index: ObjectIndex): void {
  if (def.__zone) node.userData.__zone = true
  if (def.parentId) {
    const parent = index.get(def.parentId)
    if (parent?.userData.__zone) node.userData.__logicalRoot = true
  }
}
...
export function upsertObjects(
  ...
  for (const def of defs) {
    ...
    if (def.type === 'component') registerComponentChildren(node, index)
