...
import type { ComponentContext } from '../managers/component/ComponentManager'
import type { ComponentSharedState } from '../managers/component/handlers/base/shared'
...
export function buildObjects(
  ...
  const ctx: ComponentContext = { scene, index: nodeMap, shared }
  let createdCount = 0
  let skippedCount = 0
  const debug =
  ...
  // zone 身份优先读宿主注入的 o.__zone(权威源,支持嵌套分区);无标记时回落到「root 直接子=zone」启发式。
...
/** @deprecated 用 loadModelObjects 代替。保留旧名兼容外部调用方。 */
