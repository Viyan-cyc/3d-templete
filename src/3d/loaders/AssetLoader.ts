...
export class AssetLoader {
  ...
  async loadEnvMap(url: string): Promise<THREE.Texture> {
    const rgbModule = await import('three/examples/jsm/loaders/RGBELoader.js');
    const loader = new rgbModule.RGBELoader(this._loadingManager);
    return new Promise((resolve, reject) => {
      loader.load(
        url,
        (texture) => {
...
export const getAssetLoader = (): AssetLoader => {
  if (!instance) {
