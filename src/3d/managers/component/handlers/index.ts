...
import type { CreationEntry } from '../ComponentManager'
...
const creationChain: CreationEntry[] = [
  { key: 'library', match: (d) => !!d.component?.name && hasComponent(d.component.name), handler: libraryHandler },
  { key: 'rack', match: (d) => d.component?.type === 'rack', handler: rackHandler },
  { key: 'model', match: (d) => !!d.src, handler: modelHandler },
  { key: 'primitive', match: (d) => !!d.geometry || d.type === 'mesh', handler: primitiveHandler },
